/**
 * TrainFlow — Cloudflare Worker (v1.3 Recovery)
 */
...
async function requireLearner(c, next) {
  const auth = c.req.header('Authorization'); if (!auth?.startsWith('Bearer ')) return c.json({ error: 'Unauthorized' }, 401)
  try {
    const payload = await verify(auth.slice(7), c.env.JWT_SECRET, 'HS256'); c.set('user', payload); await next()
  } catch { return c.json({ error: 'Unauthorized' }, 401) }
}
...
app.post('/api/auth/manager/login', async (c) => {
  ...
  const now = Math.floor(Date.now() / 1000)
  const token = await sign({ role: 'manager', id: user.id, name: user.name, team_id: user.team_id, iat: now, exp: now + CONSTANTS.LEARNER_JWT_EXP_SEC }, c.env.JWT_SECRET, 'HS256')
  return c.json({ token, id: user.id, name: user.name, role: 'manager', team_id: user.team_id })
})
...
app.post('/api/learners/login', async (c) => {
  ...
  const now = Math.floor(Date.now() / 1000)
  const token = await sign({ role: user.role, id: user.id, name: user.name, team_id: user.team_id, iat: now, exp: now + CONSTANTS.LEARNER_JWT_EXP_SEC }, c.env.JWT_SECRET, 'HS256')
  return c.json({ token, id: user.id, name: user.name, role: user.role, team_id: user.team_id })
})
...
app.post('/api/auth/manager/register', async (c) => {
  ...
  if (!invite) return c.json({ error: 'Invalid or used invite code' }, 400)
  ...
  const token = await sign({ id, name: body.name.trim(), role: 'manager', team_id: invite.team_id, iat: now, exp: now + CONSTANTS.LEARNER_JWT_EXP_SEC }, c.env.JWT_SECRET, 'HS256')
  return c.json({ token, id, name: body.name.trim(), role: 'manager', team_id: invite.team_id })
})

// ══════════════════════════════════════════════════════════════════════════════
//  ROUTES: TEAMS & INVITES
// ══════════════════════════════════════════════════════════════════════════════
...
app.delete('/api/admin/teams/:id', requireAdmin, async (c) => {
  const db = getDb(c.env), id = c.req.param('id')
  const check = toObj(await db.execute({ sql: 'SELECT COUNT(*) as n FROM users WHERE team_id = ?', args: [id] }))
...
app.delete('/api/admin/invites/:id', requireAdmin, async (c) => {
  const db = getDb(c.env), id = c.req.param('id')
  await db.execute({ sql: 'DELETE FROM invite_codes WHERE id = ? AND used = 0', args: [id] })
  return c.json({ ok: true })
})

// ══════════════════════════════════════════════════════════════════════════════
//  ROUTES: COURSES, COMPLETIONS & ASSIGNMENTS
// ══════════════════════════════════════════════════════════════════════════════
...
app.get('/api/courses/:id', async (c) => {
  ...
  for (const m of mods) m.questions = toObjs(await db.execute({ sql: 'SELECT * FROM questions WHERE module_id = ? ORDER BY sort_order', args: [m.id] }))
...
app.post('/api/completions', requireLearner, async (c) => {
  ...
  if (!body || !body.course_id) return c.json({ error: 'Invalid data' }, 400)
  const db = getDb(c.env), id = uid(), now = Math.floor(Date.now() / 1000), cid = body.passed ? certId() : null
  await db.execute({ sql: 'INSERT INTO completions (id, course_id, learner_id, learner_name, score, passed, completed_at, cert_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', args: [id, body.course_id, user.id, user.name, body.score || 0, body.passed ? 1 : 0, now, cid] })
  if (Array.isArray(body.responses)) {
    for (const r of body.responses) await db.execute({ sql: 'INSERT INTO question_responses (completion_id, question_id, is_correct) VALUES (?, ?, ?)', args: [id, r.question_id, r.is_correct ? 1 : 0] })
  }
...
app.get('/api/admin/stats', requireManager, async (c) => {
  ...
  const [lc, cc, cm, pr, lr] = await Promise.all([
    ...
    db.execute({ sql: `SELECT l.*, (SELECT COUNT(*) FROM completions comp WHERE comp.learner_id = l.id) as completion_count FROM users l WHERE l.role = 'learner' ${st ? ' AND l.team_id = ?' : ''} ORDER BY l.name`, args: wa })
  ])
  const p = toObj(pr)
