/**
 * TrainFlow — Cloudflare Worker (v1.4 Production-Hardened)
 */
...
app.get('/api/brand', async (c) => {
  ...
  try {
    ...
    return brand ? c.json(brand) : c.json({ org_name: 'TrainFlow', primary_color: '#2563eb', secondary_color: '#1d4ed8' })
  } catch { return c.json({ org_name: 'TrainFlow', primary_color: '#2563eb', secondary_color: '#1d4ed8' }) }
})
...
app.get('/api/courses', async (c) => {
  const db = getDb(c.env); try { const res = await db.execute('SELECT * FROM courses ORDER BY created_at'); return c.json(toObjs(res)); } catch { return c.json([]); }
})

// ── Admin/Manager Scoped Routes (Hardened) ──
app.get('/api/learners', requireManager, async (c) => {
  ...
  try {
    ...
    const res = await db.execute({ 
      sql: `SELECT l.*, 
            (SELECT COUNT(*) FROM completions c WHERE c.learner_id = l.id) as completion_count 
            FROM users l WHERE ${where.join(' AND ')} ORDER BY name`, 
      args 
    })
    return c.json(toObjs(res))
  } catch {
    // Fallback if completions or other columns missing
...
app.get('/api/admin/teams', requireManager, async (c) => {
  ...
  try {
    ...
    try { const res = await db.execute("SELECT * FROM teams ORDER BY name"); return c.json(toObjs(res)); } catch { return c.json([]); }
  }
})
...
app.get('/api/admin/stats', requireManager, async (c) => {
  ...
  try {
    ...
    return c.json({ summary: { total_learners: toObj(lc)?.n || 0, total_courses: toObj(cc)?.n || 0, completions_this_month: 0, pass_rate: 100 }, learners: toObjs(lr) })
  } catch (err) {
